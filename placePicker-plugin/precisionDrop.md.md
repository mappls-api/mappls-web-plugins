...
# Precision Drop Plugin for Mappls Web Maps
...
2. The sample codes are provided on our domain to help you understand the very basic functionality of Mappls Precision Drop Plugin. [See Sample Codes here](https://about.mappls.com/api/web-sdk/vector-plugin-example/Placepicker/mappls-placepicker-plugin) 
...
A simple plugin / widget to pick places from the map. This SDK also has integrated  Precision Drop Plugin as optional component that enables one to narrow down to picked place by searching for it first and then changing the position of the resulting point on map to fine-tune the results.
...
3. Ability to get information from Mappls  Precision Drop plugin through a callback.
4. Include the  Precision Drop Plugin with or without an interactive Map component.
...
### 1. Initializing the  Precision Drop plugin
...
```js
 /* Precision Drop plugin initialization*/
var options = {
  map: map,
  header: true,
  closeBtn: true,
...
1. `Place Options`: any object containing optional configurations for modifying the search request.
    ...
    - `closeBtn`: (boolean): to give the option to close  Precision Drop  (including hiding the top bar that has search & lower bar area that has the 'Done' button and related sub-text). Default is `true`.
    - `closeBtn_callback`: A callback method that is called when user clicks on the close button at the top left. To provide a call to action upon user closing the  Precision Drop plugin and thus providing the ability to continue towards further action by the consuming app.
    - `search`: (boolean): To enable / disable the integrated Place Search plugin. Default is `true`.
    - `topText`: The banner text to show at the top as title of the  Precision Drop plugin. Default is `Precision Drop`.
...
### 2. Calling Mappls Precision Drop for programmatically fixed text
...
### 3. Method for removing Precision Drop plugin with callback from map
#### Method
`remove()`
...
### 5. Method for getting location data from  Precision Drop plugin.

#### Description
This method is especially useful if no callback is given in options.
As per the current status of the  Precision Drop plugin's UI (map view and placement of PIN on map), the location data is returned.
...
### Response Structure of Precision Drop Data
...
That's All ! Visit the [samples](https://about.mappls.com/api/web-sdk/vector-plugin-example/Placepicker/mappls-advance-placePicker) for assistance to create a sample implementation with your own keys. 
